...
START_TEST (test_find_archetype_by_object_name)
{
    ...
    fail_unless(!strcmp(arch->clone.name,"Cloak of Magic Resistance"),
...
START_TEST (test_find_archetype_by_object_type_name)
{
    ...
    fail_unless(arch->clone.type==87,
                "Requested type 87 but got %d",arch->clone.type);
    fail_unless(!strcmp(arch->clone.name,"Cloak of Magic Resistance"),
                 "Searching for an existing arch name shoud have returned us Cloak of Magic Resistance but returned %s",
                 arch->clone.name
               );
...
START_TEST (test_create_archetype_by_object_name)
{
    ...
    fail_unless(strncmp(ob->name,ARCH_SINGULARITY,strlen(ARCH_SINGULARITY)),
                "Searching for writing pen should NOT have returned a singularity");
    fail_unless(!strncmp(ob->name,"writing pen",strlen(ob->name)),
    ...
    fail_unless(ob!=NULL,"Should never return null");
    fail_unless(strncmp(ob->name,ARCH_SINGULARITY,strlen(ARCH_SINGULARITY)),
                "Searching for writing pen of hell raiser +3 should NOT have returned a singularity");
    fail_unless(!strncmp(ob->name,"writing pen of hell raiser +3",strlen(ob->name)),
