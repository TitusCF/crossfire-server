...
object *get_pet_enemy(object * pet, rv_vector * rv)
{
        ...
        if (!(mflags & P_OUT_OF_MAP) && mflags & P_IS_ALIVE) {
            for (tmp = get_map_ob(nm, x, y); tmp != NULL; tmp = tmp->above) {
                ...
                if (QUERY_FLAG(tmp2, FLAG_ALIVE)
                    ...
                    && tmp2 != owner && can_detect_enemy(pet, tmp2, rv)) {
    ...
    if (attacker) {
        ...
        if (attacker->count == pet->attacked_by_count) {
            ...
            if (!QUERY_FLAG(attacker, FLAG_FRIENDLY)
                && on_same_map(pet, attacker)) {
                pet->enemy = attacker;
                if (check_enemy(pet, rv) != NULL)
                    return attacker;
                else
    ...
    if (owner->type == PLAYER && owner->contr->petmode != pet_defend) {
        ...
        for (i = 0; i < SIZEOFFREE; i++) {
            ...
            mflags = get_map_flags(nm, &nm, x, y, &x, &y);
            if (!(mflags & P_OUT_OF_MAP) && mflags & P_IS_ALIVE) {
                for (tmp = get_map_ob(nm, x, y); tmp != NULL;
                     tmp = tmp->above) {
                    object *tmp2 = tmp->head == NULL ? tmp : tmp->head;
                    if (QUERY_FLAG(tmp2, FLAG_ALIVE)
                        &&
...
void remove_all_pets(mapstruct * map)
{
    ...
    for (obl = first_friendly_object; obl != NULL; obl = next) {
        ...
        if (obl->ob->type != PLAYER && QUERY_FLAG(obl->ob, FLAG_FRIENDLY)
            ...
            if (obl->ob && QUERY_FLAG(obl->ob, FLAG_REMOVED)
                && FABS(obl->ob->speed) > MIN_ACTIVE_SPEED) {
                object *ob = obl->ob;
...
void follow_owner(object * ob, object * owner)
{
    ...
    for (tmp = ob; tmp != NULL; tmp = tmp->more) {
        tmp->x =
            owner->x + freearr_x[dir] + (tmp->arch ==
                                         NULL ? 0 : tmp->arch->clone.x);
        tmp->y =
            owner->y + freearr_y[dir] + (tmp->arch ==
                                         NULL ? 0 : tmp->arch->clone.y);
        tmp->map = owner->map;
    ...
    if (owner->type == PLAYER)  /* Uh, I hope this is always true... */
        draw_ext_info(NDI_UNIQUE, 0, owner, MSG_TYPE_SPELL,
                      MSG_TYPE_SPELL_PET,
...
void pet_move(object * ob)
{
    ...
    if (owner->type == PLAYER && owner->contr->petmode == pet_sad) {
        ...
        for (i = 0; i < 15; i++) {
            ...
            m = ob->map;
            if (get_map_flags(ob->map, &m, dx, dy, &dx, &dy) &
                P_OUT_OF_MAP)
                continue;
    ...
    if (!(move_ob(ob, dir, ob))) {
        ...
        for (part = ob; part != NULL; part = part->more) {
            ...
            for (ob2 = get_map_ob(m, dx, dy); ob2 != NULL;
                 ob2 = ob2->above) {
                object *new_ob;
                              ...
                              int is_golem)
{
    ...
    if ((head->type != GOLEM) && (op->type != PLAYER)) {
        ...
        if (head && head->randomitems) {
            object *htmp;
            create_treasure(head->randomitems, head,
                            GT_APPLY | GT_STARTEQUIP, 6, 0);
            for (htmp = head->inv; htmp; htmp = htmp->below)
                if (!htmp->nrof)
                    SET_FLAG(htmp, FLAG_NO_DROP);
        }
    }
...
void move_golem(object * op)
{
    ...
    if (--op->stats.hp < 0) {
        if (op->msg)
            draw_ext_info(NDI_UNIQUE, 0, op->owner, MSG_TYPE_SPELL,
                          MSG_TYPE_SPELL_PET, op->msg, op->msg);
    ...
    for (tmp = op; tmp; tmp = tmp->more) {
        sint16 x = tmp->x + freearr_x[op->direction], y =
            tmp->y + freearr_y[op->direction];
        object *victim;
        mapstruct *m;
        int mflags;
        ...
        for (victim = get_map_ob(op->map, x, y); victim;
             victim = victim->above)
...
int summon_golem(object * op, object * caster, int dir, object * spob)
{
    ...
    if (!dir)
        dir =
            find_free_spot(NULL, op->map, op->x, op->y, 1,
                           SIZEOFFREE1 + 1);

    if ((dir == -1)
        || ob_blocked(&at->clone, op->map, op->x + freearr_x[dir],
                      op->y + freearr_y[dir])) {
        draw_ext_info(NDI_UNIQUE, 0, op, MSG_TYPE_SPELL,
 ...
 */
static object *choose_cult_monster(object * pl, object * god,
                                   int summon_level)
{
    char buf[MAX_BUF];
    const char *race;
                  ...
                  const char *stringarg)
{
    ...
    if (spell_ob->other_arch) {
        /* Summoning specific item/monster */
        ...
        if (!lasttr) {
            LOG(llevError,
                "Treasurelist %s did not generate a valid entry in summon_object\n",
        ...
        /* We are summoning a god-related pet. */
        ...
        do {
            ...
            if (!ndir)
                ndir =
                    find_free_spot(mon, op->map, op->x, op->y, 1,
                                   SIZEOFFREE);
            if (ndir == -1
                || ob_blocked(mon, op->map, op->x + freearr_x[ndir],
                              op->y + freearr_y[ndir])) {
                ndir = -1;
                if (++tries == 5) {
                    draw_ext_info(NDI_UNIQUE, 0, op,
    ...
    if (spell_ob->stats.dam)
        nrof +=
            spell_ob->stats.dam + SP_level_dam_adjust(caster, spell_ob);
    ...
    for (i = 1; i <= nrof; i++) {
        ...
        if (dir) {
            ...
            ndir =
                find_free_spot(&summon_arch->clone, op->map, op->x, op->y,
                               1, SIZEOFFREE);
        ...
        if (ndir == -1
            || ob_blocked(&summon_arch->clone, op->map, op->x + x,
                          op->y + y)) {
            draw_ext_info(NDI_UNIQUE, 0, op, MSG_TYPE_SPELL,
                          MSG_TYPE_SPELL_FAILURE,
                          "There is something in the way.", NULL);
