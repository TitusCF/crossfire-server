...
static object *get_npc(const mapzone *zone) {
    int idx = RANDOM()%zone->archetypes_count;
    archetype *arch = try_find_archetype(zone->available_archetypes[idx]);

    if (!arch) {
        LOG(llevError, CITYLIFE_NAME ": get_npc() invalid archetype %s!\n", zone->available_archetypes[idx]);
        return NULL;
    }
...
static void add_npc_to_random_map(void) {
    ...
    for (test = 0; available_zones[test].count_points != -1 && count < 50; test++) {
        if ((list[count] = has_been_loaded(available_zones[test].mapname)) && (list[count]->in_memory == MAP_IN_MEMORY)) {
            zones[count] = test;
            count++;
        }
    }
